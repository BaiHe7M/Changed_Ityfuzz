...
impl BoxedABI {
    ...
    pub fn new(b: Box<dyn ABI>) -> Self {
     ...
     pub fn set_bytes(&mut self, bytes: Vec<u8>) -> bool {
        println!("Setting bytes in BoxedABI: {:?}", bytes); // 添加调试信息
        if bytes.len() < 4 {
            return false;
        }
        if self.b.get_type() == ABILossyType::TEmpty {
            if !bytes[4..].is_empty() {
                eprintln!("Error: Trying to set non-empty bytes to AEmpty type: {:?}", &bytes[4..]);
                return false;
            }
            true
        } else {
    ...
    //         return false;
    //     }
    //     if self.b.get_type() == ABILossyType::TEmpty && !bytes[4..].is_empty() {
    //         eprintln!("Error: Trying to set non-empty bytes to AEmpty type: {:?}", &bytes[4..]);
    //         return false;
    //     }
    //     self.b.set_bytes(bytes[4..].to_vec())
...
impl BoxedABI {
    ...
    pub fn from_bytes(bytes: Vec<u8>) -> Self {
        // 假设前 4 个字节是函数哈希，剩余的是 ABI 编码的参数
        if bytes.len() < 4 {
            eprintln!("Error: Bytes length is less than 4");
            return BoxedABI::default();
        }
        let function = [bytes[0], bytes[1], bytes[2], bytes[3]];
        let abi_bytes = bytes[4..].to_vec();

        // 创建一个默认的 ABI 实例
        let mut boxed_abi = BoxedABI::default();
        boxed_abi.function = function;
        boxed_abi.set_bytes(abi_bytes);
