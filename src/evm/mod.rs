...
// use crate::evm::input::EVMInputTy;
pub fn parse_function_sequence_file(file_path: &str) -> io::Result<Vec<(String, Vec<(String, String)>)>> {
    ...
    let uint256_re = Regex::new(r"^\d+$").unwrap();
    let int256_re = Regex::new(r"^-?\d+$").unwrap();
    let bool_re = Regex::new(r"^(true|false)$").unwrap();
    let string_re = Regex::new(r#"^".*"$"#).unwrap(); // 简单匹配字符串，假设字符串用双引号括起来

    for line in reader.lines() {
        let line = line?;
        println!("Reading line: {}", line); // 添加调试信息
        let key_value: Vec<&str> = line.split(':').collect();
        if key_value.len() < 2 {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid format"));
        }
        let function_name = key_value[0].to_string();
        let params_str = key_value[1..].join(":");
        ...
        for param in params {
            ...
            if param_parts.len() != 2 {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid parameter format"));
            }
            ...
            match param_type {
                "address" => {
                    if !address_re.is_match(param_value) {
                        eprintln!("Invalid address parameter: {}", param_value); // 添加调试信息
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid address parameter"));
                    }
                }
                "uint256" => {
                    if !uint256_re.is_match(param_value) {
                        eprintln!("Invalid uint256 parameter: {}", param_value); // 添加调试信息
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid uint256 parameter"));
                    }
                }
                "int256" => {
                    if !int256_re.is_match(param_value) {
                        eprintln!("Invalid int256 parameter: {}", param_value); // 添加调试信息
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid int256 parameter"));
                    }
                }
                "bool" => {
                    if !bool_re.is_match(param_value) {
                        eprintln!("Invalid bool parameter: {}", param_value); // 添加调试信息
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid bool parameter"));
                    }
                }
                "string" => {
                    if !string_re.is_match(param_value) {
                        eprintln!("Invalid string parameter: {}", param_value); // 添加调试信息
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid string parameter"));
                    }
                }
                _ => {
                    eprintln!("Unknown parameter type: {}", param_type); // 添加调试信息
                    return Err(io::Error::new(io::ErrorKind::InvalidData, "Unknown parameter type"));
                }
            }

            parsed_params.push((param_type.to_string(), param_value.to_string()));
        }

        functions.push((function_name, parsed_params));
    }
...
pub fn evm_main(mut args: EvmArgs) {
    ...
    let is_onchain = onchain.is_some();
     ...
     // 解析用户提供的函数序列和参数
     let function_sequence = if !args.function_sequence_file.is_empty() {
        parse_function_sequence_file(&args.function_sequence_file).expect("Failed to parse function sequence file")
    } else {
        Vec::new()
    };
    ...
    // 构建EVMInput
    let mut inputs = vec![];
    for (function, params) in function_sequence {
        let param_values: Vec<String> = params.into_iter().map(|(_, value)| value).collect();
        let input = EVMInput {
            input_type: EVMInputTy::from_str(&function).unwrap_or(EVMInputTy::ABI), // 确保类型匹配
            caller: EVMAddress::from_str("0x0000000000000000000000000000000000000000").unwrap(),
            contract: EVMAddress::from_str("0x0000000000000000000000000000000000000000").unwrap(),
            data: Some(BoxedABI::from_bytes(param_values.join(",").into_bytes())), // 转换为 Option<BoxedABI>
            direct_data: Bytes::new(),
            txn_value: Some(EVMU256::default()), // 使用 EVMU256::default()
            step: false,
            env: Env::default(),
    ...
    for input in inputs {
        ...
        state.add_tx_to_corpus(testcase).expect("Failed to add input to corpus");
    }
