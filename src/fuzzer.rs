...
pub static mut RUN_FOREVER: bool = false;//用于控制模糊测试器是否应该无限期运行
pub static mut ORACLE_OUTPUT: Vec<serde_json::Value> = vec![];//用于存储预言机（oracle）输出的结果
...
/// 一个实现了 ItyFuzz 逻辑的模糊测试器，使用了 LibAFL 的 [`Fuzzer`] 特性
///
/// CS: 输入语料库的调度器
/// IS: 初始状态语料库的调度器
/// F: 输入语料库的反馈（例如，覆盖率图）
/// IF: 初始状态语料库的反馈（例如，比较等）
/// I: 虚拟机输入类型
/// OF: 输入语料库的目标（例如，预言机）
/// S: 模糊测试器的状态类型
/// VS: 虚拟机状态类型
/// Addr: 地址类型（例如，H160）
/// Loc: 调用目标位置类型（例如，H160）


#[allow(clippy::type_complexity)]
#[derive(Debug)]
pub struct ItyFuzzer<VS, Loc, Addr, Out, CS, IS, F, IF, IFR, I, OF, S, OT, CI, SM>//定义结构体 ItyFuzzer
where
